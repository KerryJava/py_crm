...
def perm_check(*args,**kwargs):
    ...
    for permission_key,permission_val in  perm_dic.items():
        ...
        if per_url_name == current_url_name:
            ...
            print("no match per item cur url", current_url_name, per_url_name)
            print("no match per item key    ", permission_key,  " value ",  permission_val)

    if all(match_results):
        #主要是获取到app_name
        # app_name, *per_name = match_key.split('_')
        names_list = match_key.split('_')
        app_name = names_list[0]
        per_name = names_list[1:]
